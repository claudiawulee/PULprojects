...
describe Jack do
    ...
    it 'returns the first sentence in the original Jack rhyme' do
        jack = Jack.new()
        expect(jack.original_rhyme.start_with?("This is the house that Jack built.")).to eq(true)
    end
    #checkpoint 1 and 2 test
    it 'returns the same first sentence of the original Jack rhyme even after randomizing the poem' do
        jack = Jack.new()
        jack.random_rhyme(10)
        expect(jack.original_rhyme.start_with?("This is the house that Jack built.")).to eq(true)
    end
    #checkpoint 2 
    #occasion test fail because random chance
    it 'returns a different sentence than the original jack rhyme' do
        jack = Jack.new()
        expect(jack.random_rhyme(100).start_with?("This is the house that Jack built.")).to eq(false)
    end
    #checkpoint 3
    it 'returns the same first sentence of the original Jack rhyme' do
        jack = Jack.new()
        expect(jack.original_ending(10).start_with?("This is the house that Jack built.")).to eq(true)
    end
    #checkpoint 3
    it 'the last sentence is different from the original rhyme' do
        jack = Jack.new()
        expect(jack.original_ending(10).end_with?("TThis is the horse and the hound and the horn that 
